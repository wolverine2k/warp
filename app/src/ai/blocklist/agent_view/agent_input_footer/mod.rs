...
impl AgentInputFooter {
    ...
    fn select_attachment_file(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let file_types = picker_file_types_for_caps(
            api_type,
            &raw_model_id,
            image_setting,
            pdf_setting,
    ...
    fn handle_attachment_paste(
        ...
        if let Some(images) = &content.images {
            if !images.is_empty() {
                ...
                let best = CLIPBOARD_IMAGE_MIME_TYPES
                    .iter()
                    .find_map(|fmt| images.iter().find(|img| img.mime_type.as_str() == *fmt));
        ...
        if let Some(paths) = &content.paths {
            ...
            for (offset, path_str) in image_paths.iter().enumerate() {
                ...
                let bytes = match std::fs::read(path) {
                    Ok(b) => b,
                    Err(err) => {
                        log::warn!("AgentInputFooter paste: failed to read {path_str:?}: {err}");
                        continue;
                    }
                };
                let mime = mime_for_path(path);
                ...
                match validate(&candidate, cur_pending, &caps) {
                    ...
                    Err(rej) => {
                        let msg = rejection_message(&rej);
                        ToastStack::handle(ctx).update(ctx, |ts, ctx| {
                            ts.add_ephemeral_toast(DismissibleToast::error(msg), window_id, ctx);
                        });
                    }
                }
            }
...
impl View for AgentInputFooter {
    ...
    fn render(&self, app: &warpui::AppContext) -> Box<dyn warpui::Element> {
        ...
        let inner: Box<dyn Element> = container.finish();
        ...
        match action {
            ...
            AgentInputFooterAction::AppendValidatedAttachment(att) => {
                ...
                ctx.subscribe_to_view(&chip, move |_footer, _chip, _event, ctx| {
                    ctx.dispatch_typed_action(&AgentInputFooterAction::RemoveAttachment(index));
                });
            ...
            AgentInputFooterAction::SetThumbnailBytes {
                ...
                if attachment_still_matches {
                    ...
                    let source = warpui::assets::asset_cache::AssetSource::Raw { id: asset_id };
            ...
            AgentInputFooterAction::DragAndDropFiles(paths) => {
                ...
                for (offset, path) in paths.iter().enumerate() {
                    let bytes = match std::fs::read(path) {
                        Ok(b) => b,
                        Err(err) => {
                            log::warn!("AgentInputFooter drop: failed to read {path:?}: {err}");
                            continue;
                        }
                    };
                    let mime = mime_for_path(path);
