...
| **4c. Multimodal attachments end-to-end** 🧪 4c-1 + 4c-2 + 4c-3 code complete; live smoke pending across all three | Adds attachment support (image / pdf / audio) to BYOP agent mode end-to-end (see §14). **Three sub-phases** because the work spans three distinct subsystems: **4c-1** capabilities resolver + Off/Auto/On toggle chips per model row in settings (no enforcement yet) — *code complete, pending live smoke*; **4c-2** `AgentAttachment` data model + per-adapter wire shapes (OpenAi content-array, Anthropic image blocks, Gemini inline_data parts, Ollama `images: Vec<base64>`, DeepSeek OpenAi-shape) — *code complete, pending live smoke*; **4c-3** input-bar file picker + send-time enforcement (resolver gates Send button) + conversation history rendering of attachments — *code complete, pending live smoke*. | new `crates/ai/src/capabilities.rs`, new `crates/ai/src/attachments.rs`, per-adapter request-builder edits, new attachment input UI in the agent input bar, edits to `agent_providers_widget.rs::render_model_row` | Live test per modality per adapter |
| **4d. Dedicated compaction model** 🧪 code complete | Optional global `agents.compaction.model: Option<LLMId>` setting (see §15). New `CompactionTarget { primary_cfg, summarizer_cfg }` separates overflow detection from summarizer dispatch. New `CompactionDispatcher` reads `byop_compaction_model_provider_id` + `byop_compaction_model_id` from AISettings, resolves via `snapshot_for_request`, and builds the target. Falls back to primary on resolve failure. New "Summarization model" dropdown in the BYOP compaction settings section; inline warning when configured model is unavailable. | new `app/src/ai/compaction_dispatcher.rs`, new `crates/ai/src/local_provider/compaction/config.rs` types, edits to `local_provider_compaction.rs` + `ai_page.rs` + new `SetCompactionModel` action variant | Live test with two BYOP providers — agent on provider A, compaction on provider B; delete provider B and confirm warning + fallback |
...
## 15. Phase 4d — Dedicated compaction model 🧪 code complete
...
- **5b — Settings UI + Local Native picker wiring** 🧪 code complete on `multi-local-llm`. Per-provider "Available for orchestration" toggle (gated on `FeatureFlag::LocalLlmProvider`, default off); orchestration picker switches to `get_orchestration_llm_choices` (harness + execution-mode filtering); `validate_request` calls `validate_orchestration_model_id` as a submit-time backstop. Native (`oz`) BYOP orchestration runs end-to-end through the existing in-process BYOP dispatcher from Phase 4d. See [`plan-phase-5b.md`](plan-phase-5b.md).
- **5c — External-CLI env-var injection** 🧪 code complete on `multi-local-llm`. New `orchestration_byop_env::byop_env_for_harness` exports per-API-type+harness env vars (`ANTHROPIC_BASE_URL` / `OPENAI_BASE_URL` / etc) into the child-harness env at Local launch. `prepare_local_harness_child_launch` merges the bag into the prepared `env_vars`; `launch_local_harness_child` resolves the BYOP entry from settings before spawning. Covers Claude Code, Codex, OpenCode. Gemini CLI deferred to 5e (settings.json injection, see below). See [`plan-phase-5c.md`](plan-phase-5c.md).
- **5d — Remote credential bridge + compaction inheritance (client-side)** 🧪 code complete on `multi-local-llm`. Adds `byop_base_url` / `byop_api_type` / `compaction_model_provider_id` / `compaction_model_id` optional fields to `AgentConfigSnapshot` (the orchestration RPC wire-struct); `HarnessAuthSecretsConfig` gains `byop_auth_secret_name`. Settings UI gains the "Remote managed secret" field + "Auto-create" button (gated on Phase 5b toggle + Phase 5a reachability). `launch_remote_child` resolves the BYOP entry via `resolve_byop_for_remote_child` and populates the wire fields. **Worker-side BYOP runtime routing depends on warp.dev / Namespace server-team work** and is tracked separately in [`worker-side-byop-contract.md`](worker-side-byop-contract.md). See [`plan-phase-5d.md`](plan-phase-5d.md).
- **5e — Gemini CLI as local child harness + settings.json BYOP injection** 🧪 code complete on `multi-local-llm`. Lifts the `normalize_local_child_harness` filter that stripped `Harness::Gemini`; wires the real launch path in `prepare_local_harness_child_launch`. Per project-owner direction, BYOP api_key + base_url for Gemini are injected via `~/.gemini/settings.json` (`security.auth.apiKey` + `security.auth.endpoint`), NOT env vars — the Gemini CLI is configured via its settings file. The `byop_env_for_harness` matrix stays empty for `Harness::Gemini`. See [`plan-phase-5e-gemini-cli.md`](plan-phase-5e-gemini-cli.md).
...
| `Gemini` | Native, **Gemini CLI** | Phase 5e injects api_key + base_url via `~/.gemini/settings.json` (`security.auth.apiKey` + `endpoint`), not env vars |
