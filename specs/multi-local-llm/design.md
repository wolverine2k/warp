...
| **3b. Ollama-native adapter** 📋 drafted | Native Ollama (`/api/chat`, NDJSON streaming, `options.num_ctx`, native tool-call streaming). `ProviderAdapter` trait gains a `streaming_format()` method so the runner can branch between SSE and NDJSON drive loops. | new `local_provider/adapters/ollama/{mod,wire,request,response}.rs` + tests + NDJSON drive loop in `run.rs` | Live test against local Ollama |
